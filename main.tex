...
For the trifocal problem, $\mathcal R$ is ..
...
\label{alg:homotopy}
    ...
    $\mathcal{R}$ are the variables
	parametrized by a set of paramters $\mathcal{A}$ describing the data; Start parameters $\mathcal{A}_0$; 
	start solutions $\mathcal{R}_0$ where $F(\mathcal{R}_0;\mathcal{A}_0)=0$; Target parameters $\mathcal{A}^*$}
